// Flat key -> string per locale. UI chrome only — see stores/locale.js for
// what's deliberately left untranslated (admin content, server messages).
export const translations = {
  id: {
    // Shared
    kembaliKeMenu: 'Kembali ke Menu',
    total: 'Total',
    diskon: 'Diskon',
    cekLagi: 'Cek Lagi',
    meja: 'Meja',
    bawaPulang: 'Bawa Pulang',
    uploadBuktiDulu: 'Upload bukti pembayaran dulu',
    previewBukti: 'Preview bukti pembayaran',
    temaGelap: 'Ganti ke mode gelap',
    temaTerang: 'Ganti ke mode terang',

    // TableEntryView
    qrTidakValid: 'QR tidak valid',
    qrTidakValidDesc:
      'Kode QR ini tidak dikenali atau meja sedang tidak aktif. Coba scan ulang QR di meja kamu, atau tunjukkan layar ini ke staff terdekat untuk dibantu.',
    memeriksaMeja: 'Memeriksa meja kamu...',

    // MenuView
    scanQrTitle: 'Scan QR di meja kamu',
    scanQrDesc: 'Menu cuma bisa dibuka lewat QR yang ditempel di meja.',
    pesananSayaLabel: 'Pesanan Saya',
    panggilStaffLabel: 'Panggil Staff',
    semua: 'Semua',
    cariMenu: 'Cari menu...',
    belumAdaProduk: 'Belum ada produk.',
    menuBelumTersedia: 'Menu belum tersedia.',
    kafeTutup: 'Kafe sedang tutup',
    kafeTutupDesc:
      'Belum ada staff yang bertugas, jadi belum bisa pesan. Menu tetap bisa dilihat-lihat — halaman ini terbuka sendiri begitu staff mulai shift, tidak perlu reload.',
    kafeTutupKeranjang:
      'Isi keranjangmu tetap tersimpan. Begitu staff mulai shift, kamu bisa langsung lanjut ke pembayaran.',
    kafeTutupTombol: 'Tutup — belum bisa pesan',
    reservasiSedangTitle: 'Meja {meja} sedang dipakai untuk reservasi',
    reservasiSedangDesc:
      'Rombongan reservasi mulai pukul {jam}. Kalau kamu bukan bagian dari rombongan ini, pindah ke meja lain atau panggil staff, ya.',
    reservasiSegeraTitle: 'Meja {meja} sudah direservasi pukul {jam}',
    reservasiSegeraDesc:
      'Sebentar lagi meja ini dipakai rombongan reservasi. Kalau kamu bukan bagian dari rombongan ini, pilih meja lain atau panggil staff, ya.',
    reservasiKonfirmasi:
      'Meja {meja} sedang dipakai reservasi — pastikan kamu bagian dari rombongannya.',
    tidakAdaMenuCocok: 'Tidak ada menu yang cocok dengan "{q}".',
    habis: 'Habis',
    pilih: 'Pilih',
    tambah: 'Tambah',
    totalPesanan: 'Total Pesanan',
    checkout: 'Checkout',

    // CartView
    scanQrDulu: 'Scan QR di meja kamu dulu ya.',
    kembaliKeMenuLabel: 'Kembali ke menu',
    keranjang: 'Keranjang',
    keranjangKosong: 'Keranjang kamu masih kosong.',
    produk: 'Produk',
    catatanPlaceholder: 'Catatan (opsional), misal: less ice',
    perluDiperbarui: 'Perlu diperbarui',
    lanjutKePembayaran: 'Lanjut ke Pembayaran',

    // CheckoutView
    kembaliKeKeranjangLabel: 'Kembali ke keranjang',
    checkoutTitle: 'Checkout',
    keranjangPerluDiperbarui: 'Keranjang perlu diperbarui',
    keranjangPerluDiperbaruiDesc:
      'Ada item di keranjang yang bermasalah (stok/ketersediaan). Kembali ke keranjang untuk memperbaikinya.',
    metodePembayaran: 'Metode Pembayaran',
    metodeQris: 'QRIS',
    metodeQrisDesc: 'Scan QRIS, bayar lewat e-wallet/m-banking apa pun',
    metodeTunai: 'Tunai',
    metodeTunaiDesc: 'Bayar cash ke kasir pakai kode order',
    metodeDebit: 'Debit',
    metodeDebitDesc: 'Bayar kartu debit ke kasir pakai kode order',
    catatanPesanan: 'Catatan untuk Pesanan (opsional)',
    catatanPesananPlaceholder: 'Misal: tolong dibungkus terpisah',
    ringkasan: 'Ringkasan',
    pesanSekarang: 'Pesan Sekarang',
    kirimPesananIni: 'Kirim pesanan ini?',
    kirimPesananIniDesc:
      'Total {total}, bayar {metode}. Pastikan pesanan sudah sesuai — order yang sudah dikirim tidak bisa diubah sendiri dari sini.',
    yaPesanSekarang: 'Ya, Pesan Sekarang',

    // OrderView — status labels
    statusPending: 'Menunggu Pembayaran',
    statusWaitingVerif: 'Menunggu Verifikasi Kasir',
    statusConfirmed: 'Dikonfirmasi',
    statusCooking: 'Sedang Dibuat',
    statusReady: 'Siap Diambil',
    statusCompleted: 'Selesai',
    statusCancelled: 'Dibatalkan',
    orderTidakDitemukan: 'Order tidak ditemukan',
    // Pelacakan terikat ke perangkat pemesan (cookie tersembunyi), jadi kode
    // yang benar pun "tidak ditemukan" kalau dibuka dari HP lain.
    orderTidakDitemukanDesc:
      'Kode order salah atau sudah kedaluwarsa. Status pesanan hanya bisa dilihat dari HP yang dipakai memesan — kalau perlu, tanyakan ke kasir dengan menyebutkan kode order.',
    gagalMemuatStatus: 'Gagal memuat status pesanan',
    cobaLagiSebentar: 'Coba lagi sebentar.',
    cobaLagi: 'Coba Lagi',
    kodeOrderDisalin: 'Kode order disalin',
    gagalMenyalinKode: 'Gagal menyalin kode',
    scanQrisUntukBayar: 'Scan QRIS untuk bayar {total}',
    qrisBelumTersedia: 'QRIS belum tersedia — tunjukkan layar ini ke staff untuk bantuan.',
    uploadBuktiPembayaran: 'Upload Bukti Pembayaran',
    uploadBuktiDesc:
      'Screenshot/foto notifikasi pembayaran dari e-wallet/m-banking kamu.',
    sayaSudahBayar: 'Saya Sudah Bayar',
    kirimBuktiIni: 'Kirim bukti pembayaran ini?',
    kirimBuktiIniDesc:
      'Pastikan foto/screenshot yang dipilih benar-benar bukti pembayaran {total} untuk order ini. Kasir akan memverifikasi dari bukti ini.',
    yaSudahBayar: 'Ya, Sudah Bayar',
    sebutkanKodeKeKasir: 'Sebutkan kode order ini ke kasir',
    bayarMetodeLangsung: 'Bayar {metode} {total} langsung ke kasir.',
    tunai: 'tunai',
    debit: 'debit',
    menungguVerifikasiKasir: 'Menunggu kasir verifikasi pembayaran',
    sudahNMenit: 'Sudah {n} menit',
    pesananSelesai: 'Pesanan selesai. Terima kasih!',
    pesananSedangDiproses: 'Pesanan sedang diproses dapur.',
    pesananSiapDiambil: 'Pesanan kamu sudah siap!',
    pesananSiapDiambilDesc: 'Silakan ambil di kasir, sebutkan kode order ini.',
    notifDikonfirmasi: 'Pembayaran kamu sudah dikonfirmasi',
    notifDikonfirmasiDesc: 'Pesanan kamu masuk antrean dapur.',
    notifDimasak: 'Pesanan kamu sedang dibuat',
    notifDimasakDesc: 'Tunggu sebentar lagi ya.',
    notifSelesai: 'Pesanan kamu selesai',
    notifSelesaiDesc: 'Terima kasih sudah memesan di Popside!',
    notifDibatalkan: 'Pesanan kamu dibatalkan',
    notifDibatalkanDesc: 'Tanyakan ke staff kalau kamu merasa ini keliru.',
    aktifkanNotifikasi: 'Aktifkan',
    aktifkanNotifikasiDesc: 'Dapat notifikasi begini pas pesanan siap, meski layar ini tidak sedang dilihat.',
    nanti: 'Nanti',
    memberLabel: 'No. HP Member (opsional)',
    memberPlaceholder: 'Mis. 08123456789',
    memberDesc: 'Isi untuk kumpulin poin otomatis dan pakai diskon member. Kosongkan kalau tidak mau.',
    memberDikenali: 'Nomor ini terdaftar',
    memberPoinKamu: '{n} poin terkumpul',
    memberDiskonAktif: 'Diskon member {percent}%',
    memberDiskonSyarat: 'dari tingkatan ≥ {n} poin',
    memberBelumCukupPoin: 'Poinmu belum cukup untuk dapat diskon. Terus kumpulkan poin dari tiap pesanan.',
    memberPoinTidakDipotong: 'Poinmu tidak dipotong — diskon ini bonus dari poin yang sudah terkumpul.',
    memberAkanDapatPoin: 'Pesanan ini menambah {n} poin lagi setelah dibayar.',
    memberTidakDitemukan: 'Nomor ini belum terdaftar — akan didaftarkan otomatis setelah pesanan ini dibayar.',
    // Verifikasi OTP: diskon member hanya setelah nomor terbukti milik pemesan.
    memberPoinKeNomor: 'Poin dari pesanan ini masuk ke nomor ini setelah dibayar.',
    memberVerifikasiAjak: 'Sudah member dan punya diskon? Verifikasi nomor ini untuk memakainya.',
    memberKirimKode: 'Kirim kode verifikasi',
    memberKodeTerkirim: 'Kalau nomor ini punya diskon member, kode 6 digit dikirim ke nomor tersebut (WhatsApp/SMS). Berlaku 5 menit.',
    memberKodeLabel: 'Kode verifikasi',
    memberVerifikasiTombol: 'Verifikasi',
    memberKirimUlang: 'Kirim ulang',
    memberKirimUlangDalam: 'Kirim ulang dalam {n} dtk',
    memberVerifikasiTidakTersedia: 'Mau pakai diskon member? Tunjukkan nomormu ke kasir.',
    memberTerverifikasi: 'Nomor terverifikasi',
    memberMengecek: 'Mengecek nomor...',
    memberKurangPoin: 'Kurang {n} poin lagi untuk dapat diskon {percent}%.',
    memberKurangPoinNaik: 'Kurang {n} poin lagi untuk naik ke diskon {percent}%.',
    memberTargetPertama: 'Kumpulkan {n} poin untuk mulai dapat diskon {percent}%.',
    teleponTidakValid:
      'Nomor HP tidak valid — pakai angka saja, contoh 08123456789. Kosongkan kalau tidak mau ikut member.',
    teleponBelumLengkap: 'Nomornya belum lengkap. Lengkapi, atau kosongkan kalau tidak jadi.',
    subtotal: 'Subtotal',
    diskon: 'Diskon',
    pajak: 'Pajak',
    serviceCharge: 'Service charge',
    poinDidapat: 'Kamu dapat {n} poin dari pesanan ini',
    sudahNMenitSejakUpdate: 'Sudah {n} menit sejak status terakhir diperbarui',
    estimasiSiap: 'Biasanya siap dalam sekitar {n} menit sejak dikonfirmasi',
    detailPesanan: 'Detail Pesanan',
    catatanLabel: 'Catatan: {catatan}',
    terimaKasihMenungguVerifikasi: 'Terima kasih! Menunggu verifikasi kasir.',
    progresPesanan: 'Progres Pesanan',
    pesananDibatalkanDesc: 'Pesanan ini sudah dibatalkan.',

    // Offline queue (CheckoutView, App)
    checkoutOfflineQueued:
      'Tidak ada koneksi internet. Pesanan disimpan & akan otomatis terkirim begitu online lagi.',
    offlineOrderSent: 'Koneksi kembali — pesanan kamu berhasil terkirim.',
    offlineOrderFailed: 'Koneksi kembali, tapi pesanan gagal terkirim: {error}',
    offlineBanner: 'Tidak ada koneksi internet',

    // BillDialog
    billLabel: 'Bill',
    billTitle: 'Bill Meja {meja}',
    billKosong: 'Belum ada pesanan untuk kunjungan ini di meja ini.',
    billTotalKeseluruhan: 'Total Keseluruhan',
    billJumlahPesanan: '{n} pesanan',
    billScopeNote:
      'Cuma pesanan dari kunjungan meja ini yang sedang berjalan — bukan gabungan dari pengunjung sebelumnya.',

    // CallStaffDialog
    panggilStaffTitle: 'Panggil Staff',
    mintaAirPutih: 'Minta Air Putih',
    sendokGarpu: 'Sendok/Garpu',
    tanyaSesuatu: 'Tanya Sesuatu',
    lainnya: 'Lainnya',
    staffSegeraKeMeja: 'Staff akan segera ke meja kamu',

    // VariantPickerDialog
    wajibPilih: 'Wajib pilih',
    jumlah: 'Jumlah',
    tambahHarga: 'Tambah · {price}',

    // RecentOrdersDialog
    belumAdaPesananDiPerangkat: 'Belum ada pesanan di perangkat ini.',

    // QtyStepper
    kurangiJumlah: 'Kurangi jumlah',
    tambahJumlah: 'Tambah jumlah',
  },
  en: {
    kembaliKeMenu: 'Back to Menu',
    total: 'Total',
    diskon: 'Discount',
    cekLagi: 'Check Again',
    meja: 'Table',
    bawaPulang: 'Takeaway',
    uploadBuktiDulu: 'Please upload proof of payment first',
    previewBukti: 'Payment proof preview',
    temaGelap: 'Switch to dark mode',
    temaTerang: 'Switch to light mode',

    qrTidakValid: 'Invalid QR code',
    qrTidakValidDesc:
      "This QR code isn't recognized or the table is inactive. Try scanning your table's QR again, or show this screen to the nearest staff member for help.",
    memeriksaMeja: 'Checking your table...',

    scanQrTitle: 'Scan the QR at your table',
    scanQrDesc:
      'The menu can only be opened by scanning the QR code on your table.',
    pesananSayaLabel: 'My Orders',
    panggilStaffLabel: 'Call Staff',
    semua: 'All',
    cariMenu: 'Search menu...',
    belumAdaProduk: 'No items yet.',
    menuBelumTersedia: 'Menu not available yet.',
    kafeTutup: 'The cafe is closed',
    kafeTutupDesc:
      'No staff is on duty yet, so ordering is unavailable. You can still browse — this page unlocks by itself as soon as staff starts a shift, no reload needed.',
    kafeTutupKeranjang:
      'Your cart is saved. As soon as staff starts a shift, you can continue straight to payment.',
    kafeTutupTombol: 'Closed — ordering unavailable',
    reservasiSedangTitle: 'Table {meja} is in use for a reservation',
    reservasiSedangDesc:
      'The reserved party starts at {jam}. If you are not part of this party, please move to another table or call a staff member.',
    reservasiSegeraTitle: 'Table {meja} is reserved for {jam}',
    reservasiSegeraDesc:
      'A reserved party will use this table shortly. If you are not part of it, please choose another table or call a staff member.',
    reservasiKonfirmasi:
      'Table {meja} is in use for a reservation — make sure you are part of that party.',
    tidakAdaMenuCocok: 'No menu items match "{q}".',
    habis: 'Sold Out',
    pilih: 'Choose',
    tambah: 'Add',
    totalPesanan: 'Order Total',
    checkout: 'Checkout',

    scanQrDulu: 'Scan the QR at your table first.',
    kembaliKeMenuLabel: 'Back to menu',
    keranjang: 'Cart',
    keranjangKosong: 'Your cart is empty.',
    produk: 'Item',
    catatanPlaceholder: 'Notes (optional), e.g. less ice',
    perluDiperbarui: 'Needs updating',
    lanjutKePembayaran: 'Continue to Payment',

    kembaliKeKeranjangLabel: 'Back to cart',
    checkoutTitle: 'Checkout',
    keranjangPerluDiperbarui: 'Cart needs updating',
    keranjangPerluDiperbaruiDesc:
      'Some items in your cart have an issue (stock/availability). Go back to the cart to fix it.',
    metodePembayaran: 'Payment Method',
    metodeQris: 'QRIS',
    metodeQrisDesc: 'Scan QRIS, pay with any e-wallet/mobile banking app',
    metodeTunai: 'Cash',
    metodeTunaiDesc: 'Pay cash to the cashier using your order code',
    metodeDebit: 'Debit Card',
    metodeDebitDesc: 'Pay by debit card to the cashier using your order code',
    catatanPesanan: 'Order Notes (optional)',
    catatanPesananPlaceholder: 'E.g. please pack separately',
    ringkasan: 'Summary',
    pesanSekarang: 'Place Order',
    kirimPesananIni: 'Send this order?',
    kirimPesananIniDesc:
      'Total {total}, paying with {metode}. Make sure your order is correct — once sent, it can’t be changed yourself from here.',
    yaPesanSekarang: 'Yes, Place Order',

    statusPending: 'Awaiting Payment',
    statusWaitingVerif: 'Awaiting Cashier Verification',
    statusConfirmed: 'Confirmed',
    statusCooking: 'Being Prepared',
    statusReady: 'Ready for Pickup',
    statusCompleted: 'Completed',
    statusCancelled: 'Cancelled',
    orderTidakDitemukan: 'Order not found',
    orderTidakDitemukanDesc:
      "Wrong order code, or it has expired. An order's status can only be viewed on the phone that placed it — ask the cashier with your order code if needed.",
    gagalMemuatStatus: 'Failed to load order status',
    cobaLagiSebentar: 'Please try again shortly.',
    cobaLagi: 'Try Again',
    kodeOrderDisalin: 'Order code copied',
    gagalMenyalinKode: 'Failed to copy code',
    scanQrisUntukBayar: 'Scan QRIS to pay {total}',
    qrisBelumTersedia: 'QRIS not available yet — show this screen to a staff member for help.',
    uploadBuktiPembayaran: 'Upload Payment Proof',
    uploadBuktiDesc:
      'Screenshot/photo of your e-wallet/mobile banking payment notification.',
    sayaSudahBayar: "I've Paid",
    kirimBuktiIni: 'Send this payment proof?',
    kirimBuktiIniDesc:
      'Make sure the photo/screenshot you selected is genuinely proof of {total} payment for this order. The cashier will verify it.',
    yaSudahBayar: "Yes, I've Paid",
    sebutkanKodeKeKasir: 'Give this order code to the cashier',
    bayarMetodeLangsung: 'Pay {metode} {total} directly to the cashier.',
    tunai: 'cash',
    debit: 'debit',
    menungguVerifikasiKasir: 'Waiting for cashier to verify payment',
    sudahNMenit: '{n} min so far',
    pesananSelesai: 'Order completed. Thank you!',
    pesananSedangDiproses: 'Your order is being prepared.',
    pesananSiapDiambil: 'Your order is ready!',
    pesananSiapDiambilDesc: 'Please pick it up at the counter — mention this order code.',
    notifDikonfirmasi: 'Your payment is confirmed',
    notifDikonfirmasiDesc: 'Your order is now in the kitchen queue.',
    notifDimasak: 'Your order is being prepared',
    notifDimasakDesc: 'Just a little longer.',
    notifSelesai: 'Your order is complete',
    notifSelesaiDesc: 'Thanks for ordering at Popside!',
    notifDibatalkan: 'Your order was cancelled',
    notifDibatalkanDesc: 'Ask a staff member if you think this is a mistake.',
    aktifkanNotifikasi: 'Enable',
    aktifkanNotifikasiDesc: "Get notified like this when your order's ready, even if you're not looking at this screen.",
    nanti: 'Not now',
    memberLabel: 'Member Phone Number (optional)',
    memberPlaceholder: 'e.g. 08123456789',
    memberDesc: 'Fill in to earn points automatically and use your member discount. Leave blank to skip.',
    memberDikenali: 'This number is registered',
    memberPoinKamu: '{n} points earned so far',
    memberDiskonAktif: 'Member discount {percent}%',
    memberDiskonSyarat: 'from the ≥ {n} points tier',
    memberBelumCukupPoin: "You don't have enough points for a discount yet. Keep earning on every order.",
    memberPoinTidakDipotong: 'Your points are not deducted — this discount is a perk of the balance you have built up.',
    memberAkanDapatPoin: 'This order adds {n} more points once it is paid.',
    memberTidakDitemukan: "This number isn't registered yet — it will be once this order is paid.",
    memberPoinKeNomor: 'Points from this order go to this number once it is paid.',
    memberVerifikasiAjak: 'Already a member with a discount? Verify this number to use it.',
    memberKirimKode: 'Send verification code',
    memberKodeTerkirim: 'If this number has a member discount, a 6-digit code has been sent to it (WhatsApp/SMS). Valid for 5 minutes.',
    memberKodeLabel: 'Verification code',
    memberVerifikasiTombol: 'Verify',
    memberKirimUlang: 'Resend',
    memberKirimUlangDalam: 'Resend in {n}s',
    memberVerifikasiTidakTersedia: 'Want to use a member discount? Show your number to the cashier.',
    memberTerverifikasi: 'Number verified',
    memberMengecek: 'Checking number...',
    memberKurangPoin: '{n} more points to unlock a {percent}% discount.',
    memberKurangPoinNaik: '{n} more points to move up to a {percent}% discount.',
    memberTargetPertama: 'Collect {n} points to start getting a {percent}% discount.',
    teleponTidakValid:
      'Invalid phone number — digits only, e.g. 08123456789. Leave it blank to skip membership.',
    teleponBelumLengkap: 'That number looks incomplete. Finish it, or clear the field to skip.',
    subtotal: 'Subtotal',
    diskon: 'Discount',
    pajak: 'Tax',
    serviceCharge: 'Service charge',
    poinDidapat: 'You earned {n} points from this order',
    sudahNMenitSejakUpdate: '{n} min since the last status update',
    estimasiSiap: 'Usually ready in about {n} minutes after confirmation',
    detailPesanan: 'Order Details',
    catatanLabel: 'Note: {catatan}',
    terimaKasihMenungguVerifikasi:
      'Thank you! Waiting for cashier verification.',
    progresPesanan: 'Order Progress',
    pesananDibatalkanDesc: 'This order has been cancelled.',

    // Offline queue (CheckoutView, App)
    checkoutOfflineQueued:
      'No internet connection. Your order is saved & will be sent automatically once back online.',
    offlineOrderSent: "Connection's back — your order was sent successfully.",
    offlineOrderFailed: "Connection's back, but the order failed to send: {error}",
    offlineBanner: 'No internet connection',

    // BillDialog
    billLabel: 'Bill',
    billTitle: 'Table {meja} Bill',
    billKosong: 'No orders yet for this visit at this table.',
    billTotalKeseluruhan: 'Grand Total',
    billJumlahPesanan: '{n} orders',
    billScopeNote:
      "Only orders from this table's current visit — not combined with earlier guests.",

    panggilStaffTitle: 'Call Staff',
    mintaAirPutih: 'Request Water',
    sendokGarpu: 'Spoon/Fork',
    tanyaSesuatu: 'Ask Something',
    lainnya: 'Other',
    staffSegeraKeMeja: 'Staff will be at your table shortly',

    wajibPilih: 'Required',
    jumlah: 'Quantity',
    tambahHarga: 'Add · {price}',

    belumAdaPesananDiPerangkat: 'No orders yet on this device.',

    kurangiJumlah: 'Decrease quantity',
    tambahJumlah: 'Increase quantity',
  },
}
