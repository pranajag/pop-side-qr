...
import { CircleAlertIcon, LoaderCircleIcon, EyeIcon, EyeOffIcon, LockIcon } from '@lucide/vue'
...
async function onSubmit() {
  ...
  try {
    ...
    if (err.status === 429) {
      ...
      remainingAttempts.value = Number.isFinite(err.rateLimitRemaining) ? err.rateLimitRemaining : null
    } else {
    ...
    <div class="w-full max-w-sm space-y-6">
      ...
      <form class="space-y-4 rounded-lg border bg-card p-6" @submit.prevent="onSubmit">
        <Alert v-if="isLockedOut" variant="destructive">
          ...
          <AlertDescription>
            Terlalu banyak percobaan gagal. Coba lagi dalam <strong class="tabular-nums">{{ lockoutClock }}</strong>.
          </AlertDescription>
        </Alert>
        <Alert v-else-if="error" variant="destructive">
          ...
          <AlertDescription>
            ...
            <span v-if="remainingAttempts !== null && remainingAttempts <= 2">
              — sisa {{ remainingAttempts }}x percobaan sebelum dikunci sementara.
            </span>
          </AlertDescription>
        </Alert>
        ...
        <div class="space-y-2">
          ...
          <Input id="username" v-model="username" autocomplete="username" required autofocus :disabled="isLockedOut" />
        </div>
        ...
        <Button type="submit" class="w-full" :disabled="submitting || isLockedOut">
