...
// Client sends only { productId, qty } — price always comes from the DB,
// never trusted from the request (AGENTS.md core rule). Invalid lines are
// reported in `issues` rather than failing the whole request, so the
// frontend can show exactly what changed instead of a generic error.
